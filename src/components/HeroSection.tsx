...
export default function HeroSection() {
  ...
  return (
    <section className="relative min-h-screen flex items-end pt-20 md:pt-28 overflow-hidden">
      ...
      <div ref={ref} className="container relative z-10 pb-20 md:pb-28">
        <div className="max-w-4xl">
          <div className={`${inView ? "animate-fade-up" : "opacity-0 translate-y-8"}`}>
            <div className="industrial-tag mb-8 pulse">
          ...
          <h1
            className={`text-4xl md:text-6xl lg:text-[5.5rem] font-extrabold leading-[0.95] tracking-tight text-foreground mb-8 ${inView ? "animate-fade-up" : "opacity-0 translate-y-8"}`}
          ...
          <div
            className={`flex items-baseline gap-3 mb-8 ${inView ? "animate-fade-up" : "opacity-0 translate-y-8"}`}
          ...
          <p
            className={`text-base md:text-lg text-muted-foreground max-w-2xl mb-10 leading-relaxed ${inView ? "animate-fade-up" : "opacity-0 translate-y-8"}`}
          ...
          <div
            className={`flex flex-col sm:flex-row items-start gap-4 mb-12 ${inView ? "animate-fade-up" : "opacity-0 translate-y-8"}`}
            style={inView ? { animationDelay: "420ms" } : undefined}
          >
            <a href="#formulario" className="btn-industrial hover-scale">
              Solicitar Amostra Técnica Gratuita →
            </a>
            <div className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground border border-border px-4 py-3 bg-background/40 backdrop-blur-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-4xl">
            {solutionHighlights.map((item, i) => (
              <div
                key={item.title}
                className={`border border-border bg-background/40 backdrop-blur-sm p-5 transition-all duration-300 hover:-translate-y-1 hover:border-primary/50 hover:bg-background/55 ${inView ? "animate-fade-up" : "opacity-0 translate-y-8"}`}
