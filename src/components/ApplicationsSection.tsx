  ...
  return (
    <section id="aplicacoes" className="py-24 md:py-32 bg-card">
      <div ref={ref} className={`container transition-all duration-700 ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
        ...
        <h2 className="text-3xl md:text-4xl lg:text-5xl font-extrabold text-foreground mb-16 leading-[1.05] max-w-3xl">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {cards.map((c) => (
            <div key={c.title} className="group">
              ...
              {/* Bottom accent line */}
              <div className="mt-6 w-full h-[2px] bg-primary opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
            </div>
          ))}
        </div>
